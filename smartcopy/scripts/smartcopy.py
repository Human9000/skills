...
示例:
  ...
  python smartcopy.py E:\\work D:\\backup --reverse           (反向: 从 DST 同步到 SRC)
    ...
    parser.add_argument(
        "--file", default=None,
        help="配合 --undo-soft-delete，仅还原指定文件（相对路径或绝对路径）",
    )
    parser.add_argument(
        "--dir", default=None,
        help="配合 --undo-soft-delete，仅还原指定子目录下的文件",
    )
    parser.add_argument(
    ...
    if args.undo_soft_delete:
        ...
        if args.dir:
            print(f"  子目录过滤: {args.dir}")
        print()
        if args.file and args.dir:
            print("错误: --file 和 --dir 不能同时使用。")
            sys.exit(1)
        # --file 模式：仅还原指定文件
        if args.file:
            target = args.file
            if not os.path.isabs(target):
                target = os.path.join(os.path.abspath(args.src), target)
            if not os.path.exists(target):
                print(f"错误: 文件不存在: {target}")
                sys.exit(1)
            if not is_soft_deleted(os.path.basename(target)):
                print(f"错误: 该文件未携带软删除标记: {target}")
                sys.exit(1)
            original = SOFT_DELETE_RE.sub("", target)
            print(f"还原: {target}")
            print(f"  → {original}")
            if os.path.exists(original) and not args.yes:
        ...
        if not args.yes:
            ...
            if confirm != "y":
                print("操作已取消。")
                sys.exit(0)
        result = undo_all(args.src, force=args.yes, subdir=args.dir)
        if args.dir:
            print(f"  子目录: {args.dir}")
